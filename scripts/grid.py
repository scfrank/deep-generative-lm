...
def get_parameters(opt):
    ...
    if "mdr_example" in opt.bayes_mode:
        ...
        X_init.append([5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0])
    else:
...
def run_grid(opt, parser):
    ...
    if not opt.retest:
        custom_file = osp.join(
            opt.out_folder, "grid", "test_output_{}.txt".format(opt.bayes_mode)
        )
        ...
        custom_file = osp.join(
            opt.out_folder, "grid", "test_output_{}_retest.txt".format(opt.bayes_mode)
        )
        func = GridFunction(parameters, opt, parser, custom_file, opt.start_iter)
        func.re_test(opt.reruns)
    elif opt.retest == "novelty":
        custom_file = osp.join(
            opt.out_folder, "grid", "test_output_{}_novelty.txt".format(opt.bayes_mode)
        )
        func = GridFunction(parameters, opt, parser, custom_file, opt.start_iter)
